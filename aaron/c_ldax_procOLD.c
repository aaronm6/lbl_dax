...
void avebox_row(PyObject *args) {
	...
	printf("\t  infunc start: Py_REFCNT(n) = %li\n", Py_REFCNT(PyTuple_GetItem(args, 2)));
	PyObject *nd_s, *nd_f;
	long n;
	if (!PyArg_ParseTuple(args, "O&O&l",
		PyArray_Converter, &nd_s,
	...
	printf("\t  infunc parse: Py_REFCNT(n) = %li\n", Py_REFCNT(PyTuple_GetItem(args, 2)));
	npy_intp numel = PyArray_SIZE(nd_s);
	npy_intp numelf = PyArray_SIZE(nd_f);
	if (numel != numelf) {
		PyErr_SetString(PyExc_IndexError, "Input and output rows must have the same length");
	...
	//Py_DECREF(nd_s);
	//Py_DECREF(nd_f);
	printf("\t  infunc end: Py_REFCNT(n) = %li\n", Py_REFCNT(PyTuple_GetItem(args, 2)));
	fflush(stdout);
}
...
void rowbyrow(void (*f)(PyObject *args), PyObject *nd_i, PyObject *nd_o, long axis, PyObject *optargs) {
	...
	printf("\tRBR ---- post parsearg--- n's Py_REFCNT = %li\n", Py_REFCNT(PyTuple_GetItem(optargs,0)));
	fflush(stdout);
	if (ndim == 1) {
		//Py_INCREF(nd_i);
		//Py_INCREF(nd_o);
		...
		printf("\tRBR --- lastline --- n's Py_REFCNT = %li\n", Py_REFCNT(PyTuple_GetItem(optargs,0)));
		//Py_DECREF(nd_i);
		//Py_DECREF(nd_o);
		fflush(stdout);
	}
